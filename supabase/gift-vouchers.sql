-- Gift vouchers. Run in Supabase SQL Editor (safe to re-run).

create table if not exists gift_vouchers (
  id uuid primary key default gen_random_uuid(),
  tenant_id text not null default 'maya',
  code text not null unique,
  status text not null default 'active'
    check (status in ('pending_payment', 'active', 'exhausted', 'cancelled')),
  treatments_total integer not null check (treatments_total between 1 and 10),
  treatments_remaining integer not null check (treatments_remaining >= 0),
  unit_price_agorot integer not null default 25000,
  ...
  greeting text,
  pelecard_booking_ref text,
  appointment_ids uuid[] not null default '{}',
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint gift_vouchers_remaining_lte_total
    check (treatments_remaining <= treatments_total)
);

create unique index if not exists idx_gift_vouchers_code_upper
  on gift_vouchers (upper(code));
create index if not exists idx_gift_vouchers_tenant
  on gift_vouchers (tenant_id);
create index if not exists idx_gift_vouchers_booking_ref
  on gift_vouchers (pelecard_booking_ref);
...
begin
  ...
  if p_count < 1 or v.treatments_remaining < p_count then
    raise exception 'VOUCHER_BALANCE';
  end if;

  update gift_vouchers
  set treatments_remaining = treatments_remaining - p_count,
      status = case
        when treatments_remaining - p_count = 0 then 'exhausted'
        else 'active'
      end,
      updated_at = now()
  where id = v.id
...
begin
  update gift_vouchers
  set treatments_remaining = least(treatments_total, treatments_remaining + p_count),
      status = 'active',
      updated_at = now()
  where id = p_id;
end;
