...
export default function GiftVoucherSuccess({ voucher }) {
  ...
  const shareUrl = typeof window === "undefined" ? "" : `${window.location.origin}${bookingUrl("/gift/card")}?${new URLSearchParams({ code: voucher.code, name: voucher.recipientName || "", greeting: voucher.greeting || "", quantity: String(voucher.treatmentsTotal) })}`;
  const shareWhatsApp = () => {
  ...
  return <section className={`${clinicGlassPanel} p-6 text-center sm:p-10`} dir="rtl">
    <CheckCircle2 className="mx-auto mb-4 h-14 w-14 text-[var(--ofir-5d7f6d,#5D7F6D)]" />
    ...
    <div className="my-7 rounded-2xl border border-[#D5E0D8] bg-white/80 p-5">
      ...
      <button type="button" onClick={copy} className="mt-3 inline-flex items-center gap-2 text-sm font-semibold text-[var(--ofir-5d7f6d,#5D7F6D)]"><Copy className="h-4 w-4" />{copied ? "הועתק" : "העתקת המספר"}</button>
    </div>
